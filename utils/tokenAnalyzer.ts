...
export async function analyzeToken(
    ...
    try {
        ...
        if (metrics.devHoldings !== null && metrics.devHoldings === 0 && metrics.age < 300) {
        ...
        if (metrics.devHoldings !== null && metrics.devHoldings > 0.8) {
            score += 20;
            strengths.push('Dev still holding most tokens');
        } else if (metrics.devHoldings !== null && metrics.devHoldings > 0.5) {
            score += 12;
        } else if (metrics.devHoldings !== null && metrics.devHoldings > 0.3) {
            score += 5;
        } else if (metrics.devHoldings !== null && metrics.devHoldings > 0.1) {
            warnings.push(`Dev sold ${((1 - metrics.devHoldings) * 100).toFixed(0)}%`);
            score -= 5;
        } else if (metrics.devHoldings !== null) {
...
async function calculateMetrics(
    ...
    const effectiveName = metadata.name || token.name || "";
    const effectiveSymbol = metadata.symbol || token.symbol || "";
    const hasMetadata = effectiveName !== "" && effectiveSymbol !== "";
    
    // Metadata quality score (simple heuristic)
    let metadataQuality = 0.5;
    if (hasMetadata) {
        metadataQuality = 0.7;
        if (effectiveName.length > 3 && effectiveSymbol.length > 2) {
            metadataQuality = 0.8;
        }
        if (!effectiveName.toLowerCase().includes('test') && 
            !effectiveName.toLowerCase().includes('rug') &&
            !effectiveSymbol.toLowerCase().includes('test')) {
            metadataQuality = 0.9;
        }
    }

    let devHoldings: number | null = null;
    try {
        if (token.traderPublicKey && token.traderPublicKey !== 'SIM') {
            ...
            devHoldings = pumpData.tokenTotalSupply > 0 ? Math.min(1.0, devBalance / pumpData.tokenTotalSupply) : null;
        }
    } catch (e) { }

    const [holderCountFromRpc, holderStats] = await Promise.all([
        getHolderCount(token.mint, connection),
        getHolderStats(token.mint, connection)
    ]);
    const marketSnapshot = getMarketSnapshot(token.mint);
    const holderCount = Math.max(holderCountFromRpc || 0, marketSnapshot?.uniqueTraderCount || 0, 1);

    const holderDistribution = holderStats ? Math.min(1, holderStats.top10Concentration / 100) : Math.min(1.0, 1 / Math.max(holderCount, 1));
    ...
    return {
        ...
        devHoldings,
        age,
        observedVolume: marketSnapshot?.observedVolumeSol || 0,
        priceChange: marketSnapshot?.priceChangePercent || 0,
        hasMetadata,
        metadataQuality
    };
}
