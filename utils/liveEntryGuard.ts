...
function evaluateMomentumEntry(token: TokenData, analysis: EnhancedAnalysis, amountSol: number): EntryGuardDecision {
    ...
    if (age > 120) {
        return { status: 'reject', reason: `Aggressive continuation window already passed (${age.toFixed(0)}s old)` };
    }
    ...
    // Re-balanced for faster strikes: require (3) trades and score (45) for bypass.
    const momentum = age > 0 ? (liquidityGrowth / age) * 60 : 0;
    const hasMinCurveEvidence =
        ((analysis.bondingCurveProgress >= 1.5 && liquidityGrowth > 0) ||
        (momentum >= 12.0 && analysis.bondingCurveProgress >= 0.5 && analysis.score >= 45)) &&
        (tradeCount >= 3 || uniqueTraderCount >= 3); 
